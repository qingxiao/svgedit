...
export default {
  ...
  async init({ $, decode64, dropXMLInternalSubset }) {
    ...
    const imagelibStrings = await loadExtensionTranslation(
      svgEditor.curPrefs.lang,
    );
    ...
    async function onMessage({ origin, data: response }) {
      ...
      try {
        ...
        if (
          !allowedImageLibOrigins.includes('*') &&
          !allowedImageLibOrigins.includes(origin)
        ) {
          // Todo: Surface this error to user?
          console.log(
            `Origin ${origin} not whitelisted for posting to ${window.origin}`,
          ...
          if (svgStr) {
            if (curMeta && curMeta.name) {
              ...
              const xml = new DOMParser().parseFromString(
                dropXMLInternalSubset(response),
                'text/xml',
              ).documentElement; // lgtm [js/xml-bomb]
              title =
                $(xml)
                  .children('title')
                  .first()
                  .text() || '(SVG #' + response.length + ')';
            }
            if (curMeta) {
    ...
    function showBrowser() {
      ...
      if (!browser.length) {
        ...
        const frame = $('<iframe src="javascript:0"/>')
          .prependTo(browser)
          .hide()
        ...
        const leftBlock = $('<span>')
          .css({ position: 'absolute', top: 5, left: 10 })
          .appendTo(browser);

        const back = $(
          '<button hidden>' + imagelibStrings.show_list + '</button>',
        )
          .appendTo(leftBlock)
          .on('click touchend', function() {
        ...
        /* const type = */ $(
          '<select><option value=s>' +
            imagelibStrings.import_single +
            '</option><option value=m>' +
            imagelibStrings.import_multi +
            '</option><option value=o>' +
            imagelibStrings.open +
            '</option></select>',
        )
          .appendTo(leftBlock)
          .change(function() {

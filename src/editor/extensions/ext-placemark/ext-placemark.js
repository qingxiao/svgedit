...
export default {
  ...
  async init(S) {
    ...
    return {
      ...
      buttons: addMarkerButtons(
        strings.buttons.map((button, i) => {
      ...
      mouseDown(opts) {
        ...
        if (svgCanvas.getMode() === 'placemark') {
          ...
          const items = $('#placemarkText')
            .val()
            .split(';');
          let font = $('#placemarkFont')
            .val()
            .split(' ');
          const fontSize = Number.parseInt(font.pop());
          font = font.join(' ');

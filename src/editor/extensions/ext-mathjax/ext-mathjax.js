  ...
  async init({ $ }) {
    ...
    $.extend(uiStrings, {
      mathjax: {
        ...
        svg_save_warning:
          'The math will be transformed into a figure is ' +
      ...
      MathJax.Hub.queue.Push(function() {
        ...
        svg.find('use').each(function() {
          ...
          const id = $(this).attr('href')
            ? $(this)
                .attr('href')
                .slice(1) // Works in Chrome.
            : $(this)
                .attr('xlink:href')
                .slice(1); // Works in Firefox.
          const glymph = $('#' + id)
            .clone()
            .removeAttr('id');
          const x = $(this).attr('x');
        ...
        events: {
          async click() {
            ...
            if (mathjaxLoaded === false) {
              ...
              try {
                await Promise.resolve(require('./mathjax/MathJax.min.js')); // ?config=TeX-AMS-MML_SVG.js');
                ...
                $.alert(
                  'Failed loading MathJax. You will not be able to change the mathematics.',
                );
              }
            }
          },
